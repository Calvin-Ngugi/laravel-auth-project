                    ...
                    <div class="row rows-cols-2 ">
                        @if ($appointment->checkup_id)
                            <div class="col border p-4">
                                ...
                                <p>Nurse: {{ $appointment->checkup->nurse->first_name }}</p>
                        ...
                        @if ($appointment->diagnosis)
                            <div class="col border p-4">
                                ...
                                <p>Doctor: {{ $appointment->diagnosis->doctor->first_name }}</p>
                                <p>Disease: {{ $appointment->diagnosis['disease'] }}</p>
                                <p>Symptoms found: {{ $appointment->diagnosis['symptoms'] }}</p>
                                <p>Tests taken:
                                    @foreach ($services as $service)
                                        @if (in_array($service->id, json_decode($appointment->diagnosis['tests'])))
                                            {{ $service->name }},
                                        @endif
                                    @endforeach
                                </p>
                                ...
                                <p>Medicines prescribed:
                                    @foreach ($medicines as $medicine)
                                        @if (in_array($medicine->id, json_decode($appointment->diagnosis['treatments'])))
                                            {{ $medicine->name }},
                                        @endif
                                    @endforeach
                                </p>
            ...
            <div class="row rows-cols-2 border mt-2">
                @if ($appointment->billing_id)
                    <div class="col border p-4">
                        ...
                        <p>Accountant: {{ $appointment->billing->finance->first_name }}</p>
